...
export function ProfileSidebar({ profile }: ProfileSidebarProps) {
    return (
        <aside className="w-72 shrink-0 flex flex-col gap-6">
            ...
            {/* Meta Info List */}
            <div className="flex flex-col gap-3 text-sm text-slate-600 pt-4 border-t border-slate-100">
                {profile.location && (
                ...
                {profile.website_url && (
                    <div className="flex items-center gap-2">
                        ...
                        <a href={profile.website_url.startsWith('http') ? profile.website_url : `https://${profile.website_url}`} target="_blank" rel="noopener noreferrer" className="hover:text-blue-600 hover:underline truncate">
                ...
                {profile.reputation_score !== undefined && (
                    <div className="flex items-center gap-2">
                        ...
                        <span>Reputation: <strong className="text-slate-900 font-semibold">{profile.reputation_score.toLocaleString()}</strong></span>
                    </div>
                )}
            </div>
